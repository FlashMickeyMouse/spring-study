...
public class AnnotatedBeanDefinitionReader {
	...
	public AnnotatedBeanDefinitionReader(BeanDefinitionRegistry registry, Environment environment) {
		...
		//初始化带注解的bean定义读取器时，还顺带注册了五个bd 内置
 		//会注册处理注解配置类的bean工厂后置处理器，以便解析配置类中的信息
	...
	private <T> void doRegisterBean(Class<T> annotatedClass, @Nullable String name,
		...
		//判断这个配置类上的条件装配器@Conditional是否为true 为false则不注册这个配置bean
		...
		//处理公共的定义性注解比如@lazy @Primary这些，解析之后将其反写到bd中
		...
		//讲bd封装为bdh 因为bd没有beanName信息
		BeanDefinitionHolder definitionHolder = new BeanDefinitionHolder(abd, beanName);
		definitionHolder = AnnotationConfigUtils.applyScopedProxyMode(scopeMetadata, definitionHolder, this.registry);
		//将构建好的bd注册到defaultListableBeanFactory
